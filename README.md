...
| `packages/client` | Three.js client: GLB loading with placeholder fallback, client-side prediction with server reconciliation for the local player, 150 ms interpolation buffer for remote entities, chat/HUD/targeting UI, procedural audio |
...
**Client (WoW-style):** W/S run forward / backpedal (slower) · Space jumps · A/D turn the character, or strafe while right mouse is held · Q/E strafe · left-drag orbits the camera without turning · right-drag steers the character · both buttons = run forward · wheel zoom; the camera drifts back behind you while moving. Tab cycles nearby NPCs (selection ring shows the target); clicking targets too (forgiving hit test — a near-miss on a moving character still lands), and clicking a quest NPC within range opens their dialog directly. Esc clears. Abilities on keys 1–5 (Strike, Heavy Blow, Fireball, Heal, Kick) with server-validated range and cooldowns — definitions live in [packages/shared/src/abilities.ts](packages/shared/src/abilities.ts), and each carries a particle effect (impact burst, traveling bolt, or heal sparkles). Some NPC abilities have a **cast time**: the caster stands rooted with a yellow cast bar on its nameplate (plus a particle glow), and **Kick (5)** breaks the cast — the ability is lost for its full cooldown and the bar flashes "Interrupted!". Casts also fizzle if you dodge out of range before the bar fills. NPCs fight with ability kits of their own: goblins open with a Frenzy burst then Gnash away, and shamans cast a ranged Spark bolt and Mend themselves below half health (both casts are kickable). NPC kinds are **world data** — model, max HP, friendly/hostile, scale, ability kit, and boss settings are defined per kind in the editor's NPC Library and stored in `world.json` (`WorldDef.npcs`); older worlds get defs derived from their existing spawners at load. Enter to chat (`/g ` prefix for global, otherwise proximity chat). F interacts with the nearest quest NPC, L toggles the quest log.

## Audio

Every sound in the game — ability swings, casts, footsteps, jumps, hits, deaths, quest chimes — is synthesized live with the Web Audio API in [packages/client/src/audio/audio.ts](packages/client/src/audio/audio.ts): oscillators and filtered noise bursts, scheduled fresh each time, no sample files anywhere. Browsers block sound before a real user gesture, so it stays silent until your first click/keypress, after which everything is distance-attenuated from your position — nearby footsteps and hits are audible, far-off ones fade out, and a few things (your own hit reactions, ability-fail buzzes, quest/level-up chimes, your own death stinger) always play at full volume since they're about you specifically. Pressing an ability plays an immediate swing/whoosh before the server confirms, and the server's `combatEvent`/`castEvent` stream drives the confirmed hit, heal, cast-drone, and interrupt sounds — the same events that already drive the particle effects.

## Boss fights

Any hostile NPC kind can be a **boss** (👑 checkbox in the NPC Library): bosses get a big top-center HP frame (shown when you target them or they're in a fight) and a **phase 2** that latches once their health drops below a configurable threshold — they roar in local chat and extra abilities join their kit live. The starter boss is **Hogger** (west of spawn): phase 1 is Maul plus **Vicious Slam**, a 2.5 s cast that hits for 30 unless you Kick it (or sprint out of range to make it fizzle); at 50% HP phase 2 adds Frenzy and **Call the Pack**, a 3 s cast that summons three gnolls. Letting the summon finish means fighting the pack — kicking it costs Hogger the full 20 s cooldown. Summoned minions never respawn and despawn when their boss dies or resets.
